		...
		<div className="flex h-full flex-col">
			<Tabs defaultValue="sound-effects" className="flex h-full flex-col">
				<div className="px-3 pt-4 pb-0">
					<TabsList>
						<TabsTrigger value="sound-effects">{t("sounds.effects")}</TabsTrigger>
						<TabsTrigger value="saved">{t("sounds.saved")}</TabsTrigger>
					</TabsList>
				</div>
		...
		const fetchTopSounds = async () => {
			try {
				...
				if (!shouldIgnore) {
					...
					setError({
						error:
							error instanceof Error ? error.message : t("sounds.failedLoad"),
					});
				}
			} finally {
				if (!shouldIgnore) {
			...
			<div className="flex items-center gap-3">
				...
				<DropdownMenu>
					...
					<DropdownMenuContent align="end" className="w-56">
						...
						>
							{t("sounds.commercialOnly")}
						</DropdownMenuCheckboxItem>
						<div className="text-muted-foreground px-2 py-1.5 text-xs">
							{showCommercialOnly
								? t("sounds.commercialDesc")
								: t("sounds.allDesc")}
						</div>
					</DropdownMenuContent>
				</DropdownMenu>
				...
				>
					<div className="flex flex-col gap-4">
						...
						{isSearching && searchQuery && (
							<div className="text-muted-foreground text-sm">{t("common.searching")}</div>
						)}
						...
						{!isLoading && !isSearching && displayedSounds.length === 0 && (
							<div className="text-muted-foreground text-sm">
								{searchQuery ? t("sounds.noSoundsFound") : t("sounds.noSoundsAvailable")}
							</div>
						)}
						{isLoadingMore && (
							<div className="text-muted-foreground py-4 text-center text-sm">
								{t("sounds.loadingMore")}
							</div>
						)}
					</div>
				</ScrollArea>
		...
		return (
			<div className="bg-background flex h-full flex-col items-center justify-center gap-3 p-4">
				...
				<div className="flex flex-col gap-2 text-center">
					<p className="text-lg font-medium">{t("sounds.noSavedSounds")}</p>
					<p className="text-muted-foreground text-sm text-balance">
						{t("sounds.noSavedHint")}
					</p>
				</div>
			</div>
		);
		...
		<div className="mt-1 flex h-full flex-col gap-5">
			<div className="flex items-center justify-between">
				...
				<Dialog open={showClearDialog} onOpenChange={setShowClearDialog}>
					...
					<DialogContent>
						<DialogHeader>
							<DialogTitle>{t("sounds.clearAllTitle")}</DialogTitle>
							<DialogDescription>
								{t("sounds.clearAllDesc", { count: savedSounds.length })}
							</DialogDescription>
						</DialogHeader>
						<DialogFooter>
		...
		<div className="group flex items-center gap-3 opacity-100 hover:opacity-75">
			...
			<div className="flex items-center gap-3 pr-2">
				...
				<Button
					...
					title={isSaved ? t("sounds.removeFromSaved") : t("sounds.saveSound")}
				>
					<HugeiconsIcon
						icon={FavouriteIcon}
